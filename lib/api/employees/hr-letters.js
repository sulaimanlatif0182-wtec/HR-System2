...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (!(await requireFeature('hr_letters', res))) return;

      const body = req.body || {};

      if (body.action === 'hr_letter_send') {
        const ids = Array.isArray(body.ids) ? body.ids.map(Number).filter(Boolean) : (body.id ? [Number(body.id)] : []);
        if (!ids.length) return res.status(400).json({ error: 'Letter id or ids array is required.' });

        const { sendNotificationEmail } = await import('../../../server/email.js');
        const sent = [];
        const failed = [];

        for (const id of ids) {
          const { data: letter } = await supabase.from('hr_letters').select('*').eq('id', id).maybeSingle();
          if (!letter) { failed.push({ id, error: 'Letter not found.' }); continue; }

          const { data: employee } = await supabase.from('employees').select('id, name, email').eq('id', letter.employee_id).maybeSingle();
          if (!employee?.email) { failed.push({ id, error: 'Employee email not found.' }); continue; }
          ...
          const result = await sendNotificationEmail({
            ...
            link: '/profile',
            actionLabel: 'View HR Letter',
          }).catch(() => ({ ok: false }));

          if (result?.ok) {
            sent.push({ id, email: employee.email });
            await supabase.from('hr_letters').update({ status: 'sent', sent_at: new Date().toISOString() }).eq('id', id);
          } else {
            failed.push({ id, error: 'Email delivery failed.' });
          }
        }
        ...
        await safeInsertSystemAudit({
          ...
          new_data: { requested: ids.length, sent: sent.length, failed: failed.length },
        });

        return res.status(200).json({ sent: sent.length, failed: failed.length, details: { sent, failed } });
      }
