...
export default async function handler(req, res) {
  ...
  // Query-flag GETs (e.g. /api/employees?announcements=true) arrive here with
  // no sub-path, so path.includes('/...') never matches them. Route on the
  // parsed query instead — otherwise they fall through to employeesHandler
  // and return the employee list, which made announcement deletes (and the
  // employee-documents tab) look like successful no-ops.
  const queryFlags = req.query || {};
  const hasQuery = (...keys) => keys.some((key) => queryFlags[key] !== undefined);

  if (hasQuery('announcements', 'announcement_cleanup')) {
    return announcementsHandler(req, res);
  }

  if (hasQuery('documents')) {
