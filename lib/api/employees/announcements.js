...
import { notifyAnnouncementPublished } from '../../../server/notify.js';
...
export default async function handler(req, res) {
  ...
  let authUser = null;
  if (req.query?.announcement_cleanup !== '1') {
  ...
  try {
    if (req.method === 'GET') {
      if (req.query.announcement_cleanup === '1') {
        ...
        for (const row of oldRows || []) {
          const { error: delError } = await supabase.from('company_announcements').delete().eq('id', row.id);
          if (!delError) deleted += 1;
        }
        await safeInsertSystemAudit({ module: 'announcements', action: 'announcement_auto_cleanup', changed_by: null, changed_by_name: 'System (cron)', new_data: { deleted, cutoff: cutoff.toISOString().slice(0, 10) } });
    ...
    if (req.method === 'POST') {
      ...
      if (!(await requireFeature('announcements', res))) return;
      ...
      if (body.action === 'announcement_save') {
        ...
        if (!body.id) {
          await safeNotify(notifyAnnouncementPublished, data);
        }
