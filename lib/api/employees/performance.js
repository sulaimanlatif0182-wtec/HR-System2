...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      // Disabled Performance toggle stops ALL writes (incl. acknowledge).
      // Reads stay open and stored data is kept, so re-enabling resumes instantly.
      if (!(await requireFeature('performance', res))) return;

      // Acknowledge actions are allowed for the owner (employee/staff view-only flow).
      // All create/edit/delete actions are admin-only — managers are view-only.
      if (body.action === 'performance_acknowledge' || body.action === 'evaluation_acknowledge') {
        // fall through to per-action owner checks below (no admin gate here)
      } else if (!assertAdmin(authUser, res)) return;
      ...
      if (body.action === 'evaluation_save') {
        if (authUser.role !== 'admin') return res.status(403).json({ error: 'Only admin can submit evaluations. Managers are view-only.' });
      ...
      if (body.action === 'evaluation_delete') {
        if (authUser.role !== 'admin') return res.status(403).json({ error: 'Only admin can delete evaluations. Managers are view-only.' });
        if (!body.id) return res.status(400).json({ error: 'id is required.' });
