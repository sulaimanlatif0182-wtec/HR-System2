  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (body.action === 'import_employees') {
        return await import('./imports.js').then(m => m.default(req, res, { supabase, authUser, body }));
      }
      ...
      if (!(await requireFeature('employees', res))) return;
    ...
    if (req.method === 'PUT') {
      ...
      const selfBody = req.body || {};

      // Self-service: any authenticated user may set their OWN birthday.
      // The field allowlist is strict — nothing else can change this way.
      if (selfBody.action === 'update_own_birthday') {
        const targetId = Number(selfBody.id || authUser.id);

        if (!targetId || targetId !== Number(authUser.id)) {
          return res.status(403).json({ error: 'You can only update your own birthday.' });
        }

        const parsed = parseBirthday(selfBody.date_of_birth);
        ...
        const { data, error } = await supabase
          .from('employees')
          .update({ date_of_birth: parsed.value })
          .eq('id', targetId)
          .select()
          .single();

        if (error) {
          return res.status(500).json({ error: friendlyDatabaseError(error, 'Failed to save birthday.') });
        }

        await safeInsertSystemAudit({
          module: 'employees',
          action: 'employee_birthday_self_update',
          record_id: targetId,
          employee_id: targetId,
          changed_by: authUser?.id || null,
          changed_by_name: authUser?.name || null,
          new_data: { date_of_birth: parsed.value },
        });

        return res.status(200).json({ success: true, employee: publicEmployee(data) });
      }
