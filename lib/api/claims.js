...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      // Dashboard view: everything still awaiting a decision, in one call.
      if (req.query?.pending === 'true') {
        query = query.not('status', 'in', '(approved,rejected,cancelled,paid)');
      }
    ...
    if (req.method === 'PUT') {
      ...
      if (action === 'cancel') {
        if (!(await requireFeature('claims_request', res))) return;
        if (!actor_id) {
          return res.status(400).json({
