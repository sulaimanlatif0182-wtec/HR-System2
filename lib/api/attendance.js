...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      } else if (req.query?.employee_id && Number.isInteger(Number(req.query.employee_id))) {
        attendanceQuery = attendanceQuery.eq('employee_id', Number(req.query.employee_id));
      }

      // Optional date scoping (YYYY-MM-DD) so callers like the dashboard can
      // fetch a single day instead of downloading the entire history table.
      const dateParamRe = /^\d{4}-\d{2}-\d{2}$/;
      if (typeof req.query?.date === 'string' && dateParamRe.test(req.query.date)) {
        attendanceQuery = attendanceQuery.eq('date', req.query.date);
      }
      if (typeof req.query?.date_from === 'string' && dateParamRe.test(req.query.date_from)) {
        attendanceQuery = attendanceQuery.gte('date', req.query.date_from);
      }
      if (typeof req.query?.date_to === 'string' && dateParamRe.test(req.query.date_to)) {
        attendanceQuery = attendanceQuery.lte('date', req.query.date_to);
      }
      if (req.query?.limit) {
        const capped = Math.min(Math.max(Number(req.query.limit) || 0, 1), 2000);
        attendanceQuery = attendanceQuery.limit(capped);
      }
    ...
    // =========================
    if (req.method === 'PUT') {
      if (!(await requireFeature('attendance', res))) return;
      const body = req.body || {};
