...
// Soft-gate helper: when a feature toggle is OFF, reject the write with 403
// but leave stored data untouched so re-enabling resumes instantly.
// Returns true when enabled (caller continues), false when blocked (response sent).
export async function requireFeature(key, res, message) {
  if (await isFeatureEnabled(key)) return true;
  const def = defaultFlag(key);
  res.status(403).json({
    error: message || `${def?.label || key} is currently disabled by Admin.`,
  });
  return false;
}
