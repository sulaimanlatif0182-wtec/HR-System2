...
import { parseAccountEmail, isCompanyEmail, COMPANY_EMAIL_DOMAIN } from './validators.js';
import { sendNotificationEmail } from '../server/email.js';
import { recordExists, safeInsertSystemAudit } from '../api/employees.js';
import {
  cleanString,
  normalizeEmail,
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    if (!isCompanyEmail(email)) {
      errors.push({
        row: rowNumber,
        email,
        message: `Only emails from @${COMPANY_EMAIL_DOMAIN} are allowed to create an account.`,
      });
      continue;
    }
