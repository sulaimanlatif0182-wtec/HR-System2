...
export async function requireAuth(req, res) {
  ...
  if (token && verifyClient) {
    ...
    if (!error && data?.user) {
      // Employees are linked to auth users by EMAIL, not by id. employees.id is a
      // bigint while auth.uid() is a UUID, so matching on id can never succeed.
  ...
  // No (valid) Supabase JWT: fall back to the worker kiosk session cookie set
  // by /api/auth/worker-login. Workers have no Supabase auth account, so their
  // requests carry no Bearer token — the HttpOnly cookie is their credential.
  const worker = await verifyWorkerSession(req);
  ...
  if (worker) {
    req.user = {
      ...
      employee_no: worker.employee_no ?? null,
